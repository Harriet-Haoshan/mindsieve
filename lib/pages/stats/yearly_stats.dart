...
/// 年视图：1月～12月每月总使用时长柱状图
class YearlyStatsView extends StatefulWidget {
  const YearlyStatsView({super.key});
...
class _YearlyStatsViewState extends State<YearlyStatsView> {
  ...
  Future<void> _loadData() async {
    ...
    final data = await _stats.getYearMonthlyTotals();
    if (!mounted) return;
    setState(() {
      _data = data;
      _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          _buildHeaderCard('今年总时长', formatDuration(total)),
          ...
          BarTotalsChart(data: _data, labelInterval: 1, rodWidth: 18),
